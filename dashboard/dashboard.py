...
def count_by_day_df(day_df):
    day_df_count = day_df.query(str('datetime >= "2011-01-01" and datetime < "2012-12-31"'))
    return day_df_count
...
def create_rfm_df(df):
    ...
    rfm_df = days_df.groupby('registered').agg({
...
main_df_days = days_df[(days_df["datetime"] >= str(start_date)) & 
                       (days_df["datetime"] <= str(end_date))]

main_df_hour = hours_df[(hours_df["datetime"] >= str(start_date)) & 
                        (hours_df["datetime"] <= str(end_date))]
...
monthly_counts = main_df_days.groupby(by=["year", "month"]).agg({
    "total_count": "sum"
}).reset_index()
...
seasonal_usage = season_rent_df.groupby('season')[['registered', 'casual']].sum().reset_index()
